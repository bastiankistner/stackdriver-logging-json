...
const fluentBit130_1 = require("../fluentBit130");
const std_1 = require("../std");
const entry_1 = require("./entry");
describe('test std formatter', () => {
    test.only('full example', () => {
        console.log(entry_1.entry);
        expect(entry_1.entry).toMatchInlineSnapshot({
            data: {
	...
	`);
        const stdEntry = std_1.entryToStd(entry_1.entry);
        console.log(JSON.stringify(stdEntry, null, 4));
        const stdFbEntry = fluentBit130_1.entryToFluentBit130(entry_1.entry);
        console.log(JSON.stringify(stdFbEntry, null, 4));
    });
});
//# sourceMappingURL=std.test.js.map